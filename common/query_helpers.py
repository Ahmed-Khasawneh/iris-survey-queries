...
def ipeds_reporting_period_mcr(spark, survey_info_in, default_values_in, ipeds_client_config_in=None):
    ...
    if ipeds_reporting_period_in.rdd.isEmpty() == False:
        ...
        ipeds_reporting_period_in = (ipeds_reporting_period_in
                                     ...
                                     .filter(((upper(ipeds_reporting_period_in.surveySection).isin(
            default_values_in['current_survey_sections']) == True) |
                                              ((col('report_prior_year') == 'Y') & (
                                                          upper(ipeds_reporting_period_in.surveySection).isin(
                                                              default_values_in['prior_survey_sections']) == True)) |
                                              ((col('report_prior_year_2') == 'Y') & (
                                                          upper(ipeds_reporting_period_in.surveySection).isin(
                                                              default_values_in['prior_2_survey_sections']) == True))))
                                     .select(
            ...
            coalesce(to_timestamp(col('recordActivityDate')), to_timestamp(lit('9999-09-09'))).alias(
                'recordActivityDate'),
            ...
            when(upper(ipeds_reporting_period_in.surveySection).isin(default_values_in['current_survey_sections']),
                 lit('CY'))
                .when(upper(ipeds_reporting_period_in.surveySection).isin(default_values_in['prior_survey_sections']),
                      lit('PY1'))
                .when(upper(ipeds_reporting_period_in.surveySection).isin(default_values_in['prior_2_survey_sections']),
                      lit('PY2')).alias('yearType'),
    ...
    if (academic_term_in.rdd.isEmpty() == False) & (ipeds_reporting_period_in.rdd.isEmpty() == False):
        ipeds_reporting_period_2 = (academic_term_in
            ...
            when(col('termClassification') == 'Standard Length', lit(1))
                .when(col('termClassification').isNull(),
                      when(col('termType').isin('Fall', 'Spring'), lit(1)).otherwise(lit(2)))
            ...
            coalesce(to_date(academic_term_in.censusDate, 'YYYY-MM-DD'),
                     (when(col('termType') == 'Fall', lit(default_values_in['default_fall_census'])))).alias(
                'repPerCensusDate'),
            ...
            when(ipeds_reporting_period_in.report_term_or_date == 'D',
                 ipeds_reporting_period_in.reportStartDate).otherwise(
                to_date(academic_term_in.minTermStart, 'YYYY-MM-DD')).alias('repPerReportStartDate'),
            when(ipeds_reporting_period_in.report_term_or_date == 'D',
                 ipeds_reporting_period_in.reportEndDate).otherwise(
                to_date(academic_term_in.maxTermEnd, 'YYYY-MM-DD')).alias('repPerReportEndDate'),
            ...
            academic_term_in.financialAidYear.alias('repPerFinancialAidYear'))
                                    .withColumn('repPerDummyDate',
                                                to_date(to_timestamp(lit('9999-09-09')), 'YYYY-MM-DD'))
                                    .withColumn('repPerSnapShotMaxDummyDate',
                                                to_date(to_timestamp(lit('9999-09-09')), 'YYYY-MM-DD'))
                                    .withColumn('repPerSnapShotMinDummyDate',
                                                to_date(to_timestamp(lit('1900-09-09')), 'YYYY-MM-DD'))
                                    .withColumn('repPerEquivCRHRFactor', expr(
            "(coalesce(repPerRequiredFTCreditHoursUG/coalesce(repPerRequiredFTClockHoursUG, repPerRequiredFTCreditHoursUG), 1))"))
                                    .withColumn('rowNum', row_number().over(Window
            .partitionBy(
            col('repPerTermCode'),
            col('repPerPartOfTermCode'))
            .orderBy(
            when((array_contains(col('repPerTags'), default_values_in['cohort_academic_fall'])) & (
                        col('repPerTermType') == 'Fall'), lit(1)).otherwise(lit(2)).asc(),
            when((array_contains(col('repPerTags'), default_values_in['cohort_academic_spring'])) & (
                        col('repPerTermType') == 'Spring'), lit(1)).otherwise(lit(2)).asc(),
            when((array_contains(col('repPerTags'), default_values_in['cohort_academic_pre_fall_summer'])) & (
                        col('repPerTermType') == 'Summer'), lit(1)).otherwise(lit(2)).asc(),
            when((array_contains(col('repPerTags'), default_values_in['cohort_academic_post_spring_summer'])) & (
                        col('repPerTermType') == 'Summer'), lit(1)).otherwise(lit(2)).asc(),
            when((col('repPerSnapshotDate') <= to_date(date_add(col('repPerCensusDate'), 3), 'YYYY-MM-DD'))
                 & (col('repPerSnapshotDate') >= to_date(date_sub(col('repPerCensusDate'), 1), 'YYYY-MM-DD')),
                 lit(1)).otherwise(lit(2)).asc(),
            when(col('repPerSnapshotDate') > col('repPerCensusDate'), col('repPerSnapshotDate')).otherwise(
                col('repPerSnapShotMaxDummyDate')).asc(),
            when(col('repPerSnapshotDate') < col('repPerCensusDate'), col('repPerSnapshotDate')).otherwise(
                col('repPerSnapShotMinDummyDate')).desc())))
                                    .filter(col('rowNum') == 1))
        ...
        academic_term_reporting_refactor_out = (ipeds_reporting_period_2
                                                ...
                                                .withColumn('repPerTermTypeNew', when(
            (col('repPerTermType') == 'Summer') & (col('repPerTermClassification') != 'Standard Length'),
    ...
    if campus_in.rdd.isEmpty() == False:
        campus_in = (campus_in
            ...
            to_date(col('snapshotDate'), 'YYYY-MM-DD').alias('snapshotDate'))
                     ...
                     .withColumn('useSnapshotDatePartition',
                                 when(col('snapshotDateFilter').isNull(), col('snapshotDateTimestamp')).otherwise(
                                     lit(None)))
...
def person_cohort_mcr(spark, cohort_df_in):
    person_in = (cohort_df_in
        ...
        (((coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) != col(
            'repPerDummyDate')) &
          (coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) <= col(
              'repPerCensusDate')))
         | (coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) == col(
                    'repPerDummyDate'))), 'left')
                 .select(
        cohort_df_in['*'],
...
def academic_track_cohort_mcr(spark, cohort_df_in, academic_term_in, priority_type_in='first'):
    ...
    academic_track_in = (cohort_df_in
        ...
        (((coalesce(to_date(col('fieldOfStudyActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) != col(
            'repPerDummyDate')) &
          (coalesce(to_date(col('fieldOfStudyActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) <= col(
              'repPerCensusDate')))
         | ((coalesce(to_date(col('fieldOfStudyActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) == col(
        ...
        to_timestamp(coalesce(col('fieldOfStudyActionDate'), col('repPerDummyDate'))).alias(
            'fieldOfStudyActionDateTimestamp'),
        coalesce(to_date(col('fieldOfStudyActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')).alias(
            'fieldOfStudyActionDate'),
        to_timestamp(coalesce(col('recordActivityDate'), col('repPerDummyDate'))).alias('recordActivityDateTimestamp'),
        coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')).alias('recordActivityDate'),
        to_timestamp(col('snapshotDate')).alias('snapshotDateTimestamp'),
        to_date(col('snapshotDate'), 'YYYY-MM-DD').alias('snapshotDate'))
...
def degree_cohort_mcr(spark, cohort_df_in):
    degree_in = (cohort_df_in
        ...
        col('degreeLevel'),
        coalesce(col('isNonDegreeSeeking_calc'), col('isNonDegreeSeeking'), lit(False)).alias(
            'isNonDegreeSeeking_final'),
        to_timestamp(coalesce(col('recordActivityDate'), col('repPerDummyDate'))).alias('recordActivityDateTimestamp'),
        coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')).alias('recordActivityDate'),
        to_timestamp(col('snapshotDate')).alias('snapshotDateTimestamp'),
        to_date(col('snapshotDate'), 'YYYY-MM-DD').alias('snapshotDate'))
...
def financial_aid_cohort_mcr(spark, default_values_in, cohort_df_in, ipeds_client_config_in):
    ...
    financial_aid_snapshot = (spark.sql(
        "select distinct to_date(snapshotDate, 'YYYY-MM-DD') ENTSnapshotDate, snapshotDate ENTSnapshotDateTimestamp, tags from financialAid")
                              ...
                              .withColumn('snapShotMaxDummyDate',
                                          to_date(to_timestamp(lit('9999-09-09')), 'YYYY-MM-DD'))
                              .withColumn('snapShotMinDummyDate',
                                          to_date(to_timestamp(lit('1900-09-09')), 'YYYY-MM-DD'))
                              .withColumn('ENTRowNum', row_number().over(Window
        .orderBy(
        ...
        when((col('ENTSnapshotDate') <= date_add(col('end_date'), 5))
             & (col('ENTSnapshotDate') >= date_sub(col('end_date'), 5)), col('ENTSnapshotDateTimestamp')).otherwise(
            col('snapShotMinDummyDate')).desc(),
        when(col('ENTSnapshotDate') > col('end_date'), col('ENTSnapshotDate')).otherwise(
            col('snapShotMaxDummyDate')).asc(),
        when(col('ENTSnapshotDate') < col('end_date'), col('ENTSnapshotDate')).otherwise(
            col('snapShotMinDummyDate')).desc()
    )))
                              .filter(col('ENTRowNum') == 1)
    ...
    cohort_financial_aid = (cohort_df_in
                            .join(
        financial_aid_in,
        (cohort_df_in.personId == financial_aid_in.personId) & (
                    cohort_df_in.financialAidYear == financial_aid_in.financialAidYear), 'left')
                            .select(
        cohort_df_in['*'],
    ...
    if survey_type == 'SFA':
        cohort_with_financial_aid = (cohort_financial_aid
                                     .withColumn('ipeds_survey_amount',
                                                 when((col('IPEDSFinancialAidAmount').isNotNull()) & (
                                                             col('IPEDSFinancialAidAmount') > 0),
                                                      col('IPEDSFinancialAidAmount'))
                                                 .when(col('fundType') == 'Loan', col('acceptedAmount'))
                                                 .when(col('fundType').isin('Grant', 'Scholarship'),
                                                       col('offeredAmount'))
                                                 .when(col('fundType') == 'Work Study', col('paidAmount'))
                                                 .otherwise(col('IPEDSFinancialAidAmount')))
                                     .withColumn('awardedAidInd', when(
            (~col('awardStatus').isin('Source Denied', 'Cancelled')) & (col('ipeds_survey_amount') > 0),
        ...
        student_fa_totals = (cohort_with_financial_aid
            ...
            col('faTermCode'),
            col('livingArrangement'),
            when((col('fundType') == 'Loan') & (col('fundSource') == 'Federal'), col('ipeds_survey_amount')).otherwise(
                lit(0)).alias('federalLoan'),
            when((col('fundType').isin('Grant', 'Scholarship')) & (col('fundSource') == 'Federal'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('federalGrantSchol'),
            when((col('fundType') == 'Work Study') & (col('fundSource') == 'Federal'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('federalWorkStudy'),
            when((col('fundType') == 'Loan') & (col('fundSource').isin('State', 'Local')),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('stateLocalLoan'),
            when((col('fundType').isin('Grant', 'Scholarship')) & (col('fundSource').isin('State', 'Local')),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('stateLocalGrantSchol'),
            when((col('fundType') == 'Work Study') & (col('fundSource').isin('State', 'Local')),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('stateLocalWorkStudy'),
            when((col('fundType') == 'Loan') & (col('fundSource') == 'Institution'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('institutionLoan'),
            when((col('fundType').isin('Grant', 'Scholarship')) & (col('fundSource') == 'Institution'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('institutionGrantSchol'),
            when((col('fundType') == 'Work Study') & (col('fundSource') == 'Institution'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('institutionalWorkStudy'),
            when((col('fundType') == 'Loan') & (col('fundSource') == 'Other'), col('ipeds_survey_amount')).otherwise(
                lit(0)).alias('otherLoan'),
            when((col('fundType').isin('Grant', 'Scholarship')) & (col('fundSource') == 'Other'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('otherGrantSchol'),
            when((col('fundType') == 'Work Study') & (col('fundSource') == 'Other'),
                 col('ipeds_survey_amount')).otherwise(lit(0)).alias('otherWorkStudy'),
            ...
            when((col('fundType').isin('Grant', 'Scholarship')), col('ipeds_survey_amount')).otherwise(lit(0)).alias(
                'allGrantSchol'),
            when((col('fundType') == 'Loan'), col('ipeds_survey_amount')).otherwise(lit(0)).alias('allLoan'),
            when((col('fundType').isin('Grant', 'Scholarship')) & (col('fundSource') == 'Federal') & (
                        col('isPellGrant') == False), col('ipeds_survey_amount')).otherwise(lit(0)).alias(
                'nonPellFederalGrantSchol'),
            ...
            when((col('isGroup2Ind') == 1) & (col('fundType').isin('Loan', 'Grant', 'Scholarship')) & (
                col('fundSource').isin('Federal', 'State', 'Local', 'Institution')), col('ipeds_survey_amount')).alias(
                'group2bTotal'),
            when((col('isGroup2Ind') == 1) & (col('fundType').isin('Grant', 'Scholarship')) & (
                col('fundSource').isin('Federal', 'State', 'Local', 'Institution')), col('ipeds_survey_amount')).alias(
                'group3Total'),
            when((col('isGroup2Ind') == 1) & (col('fundType').isin('Grant', 'Scholarship')) & (
                col('fundSource').isin('Federal', 'State', 'Local', 'Institution')) & (
                     ~col('fundCode').isin(col('sfaCaresAct1'), col('sfaCaresAct2'))),
                 col('ipeds_survey_amount')).alias('group3Total_caresAct'),
            when((col('isGroup2Ind') == 1) & (col('isTitleIV') == True), col('ipeds_survey_amount')).alias(
                'group4Total'),
            when((col('isGroup2Ind') == 1) & (col('isTitleIV') == True) & (
                ~col('fundCode').isin(col('sfaCaresAct1'), col('sfaCaresAct2'))), col('ipeds_survey_amount')).alias(
                'group4Total_caresAct'))
            .groupBy(
            'personId',
            # 'financialAidYear',
            'yearType')
        ...
        cohort_with_financial_aid = (cohort_financial_aid
                                     .withColumn('ipeds_survey_amount',
                                                 when((col('IPEDSOutcomeMeasuresAmount').isNotNull()) & (
                                                             col('IPEDSOutcomeMeasuresAmount') > 0),
                                                      col('IPEDSOutcomeMeasuresAmount'))
        ...
        student_fa_totals = (cohort_with_financial_aid
            ...
            when(col('isSubsidizedDirectLoan') == True, col('ipeds_survey_amount')).otherwise(lit(0)).alias(
                'subsidLoanAmt'))
            .groupBy(
            col('personId'),
            # col('financialAidYear'),
...
def military_benefit_mcr(spark, default_values_in, benefit_type_in):
    ...
    military_benefit_snapshot = (spark.sql(
        "select distinct to_date(snapshotDate, 'YYYY-MM-DD') ENTSnapshotDate, snapshotDate ENTSnapshotDateTimestamp, tags from militaryBenefit")
    ...
    military_benefit_in = (military_benefit_snapshot
        ...
        (col('snapshotDate') == col('ENTSnapshotDateTimestamp')) &
        (((coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('dummyDate')) != col('dummyDate')) &
          (coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('dummyDate')).between(col('start_date'),
                                                                                                col('end_date'))))
         | (coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('dummyDate')) == col('dummyDate'))) &
    ...
    military_benefit_level = (military_benefit_in
        ...
        col('benefitAmount'),
        col('ENTSnapshotDateTimestamp'),
        when(col('studentLevel').isin('Masters', 'Doctorate', 'Professional Practice Doctorate'), lit(2)).otherwise(
            lit(1)).alias('studentLevel'),
        col('snapshotDate').alias('snapshotDateTimestamp'),
        col('recordActivityDate').alias('recordActivityDateTimestamp'),
        col('snapShotMaxDummyDate'),
        col('snapShotMinDummyDate'))
        ...
        when(col('snapshotDateTimestamp') > col('ENTSnapshotDateTimestamp'), col('snapshotDateTimestamp')).otherwise(
            col('snapShotMaxDummyDate')).asc(),
        when(col('snapshotDateTimestamp') < col('ENTSnapshotDateTimestamp'), col('snapshotDateTimestamp')).otherwise(
            col('snapShotMinDummyDate')).desc(),
        col('recordActivityDateTimestamp').desc())))
...
def award_for_completions_mcr(spark, survey_info_in, default_values_in, reporting_periods_in):
    award_snapshot = (spark.sql(
        "select distinct to_date(snapshotDate, 'YYYY-MM-DD') ENTSnapshotDate, snapshotDate ENTSnapshotDateTimestamp, tags from award")
        ...
        .withColumn('ENTRowNum', row_number().over(Window
        .orderBy(
        when((col('term_or_date') == 'D') & (array_contains(col('tags'), default_values_in['cohort_program'])),
             lit(1)).otherwise(lit(2)).asc(),
        when((col('term_or_date') == 'T') & (array_contains(col('tags'), default_values_in['cohort_academic_fall'])),
             lit(1)).otherwise(lit(2)).asc(),
        when((col('ENTSnapshotDate') <= date_add(col('end_date'), 15))
             & (col('ENTSnapshotDate') >= date_sub(col('start_date'), 15)), lit(1)).otherwise(lit(2)).asc(),
        when((col('ENTSnapshotDate') <= date_add(col('end_date'), 5))
    ...
    award = (reporting_periods_in
        ...
        (((coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) != col(
            'repPerDummyDate')) &
          (coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')).between(
              col('repPerReportStartDate'), col('repPerReportEndDate'))))
         | (coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) == col(
                    'repPerDummyDate'))) &
        (((col('term_or_date') == 'D') & (
            to_date(col('awardedDate'), 'YYYY-MM-DD').between(col('repPerReportStartDate'),
...
def course_type_counts(spark, survey_info_in, default_values_in, ipeds_client_config_in=None, academic_term_in=None,
                       reporting_periods_in=None):
    if not ipeds_client_config_in:
    ...
    if not reporting_periods_in:
        if not ipeds_reporting_period_in:
            ipeds_reporting_period_in = ipeds_reporting_period_mcr(spark=spark, survey_info_in=survey_info_in,
                                                                   default_values_in=default_values_in, )
        reporting_periods_in = reporting_periods(spark=spark, survey_info_in=survey_info_in,
                                                 default_values_in=default_values_in,
                                                 ipeds_reporting_period_in=ipeds_reporting_period_in,
                                                 academic_term_in=academic_term_in)
    ...
    if (reporting_periods_in.rdd.isEmpty() == False) & (registration_in.rdd.isEmpty() == False):
        ...
        registration = (registration_in
            ...
            (((coalesce(to_date(col('registrationStatusActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) != col(
                'repPerDummyDate')) &
              (coalesce(to_date(col('registrationStatusActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) <= col(
                  'repPerCensusDate')))
             | ((coalesce(to_date(col('registrationStatusActionDate'), 'YYYY-MM-DD'), col('repPerDummyDate')) == col(
            ...
            to_timestamp(coalesce(registration_in.registrationStatusActionDate, col('repPerDummyDate'))).alias(
                'regStatusActionDateTimestamp'),
            coalesce(to_date(registration_in.registrationStatusActionDate, 'YYYY-MM-DD'), col('repPerDummyDate')).alias(
                'regStatusActionDate'),
            to_timestamp(coalesce(col('recordActivityDate'), col('repPerDummyDate'))).alias(
                'recordActivityDateTimestamp'),
            coalesce(to_date(col('recordActivityDate'), 'YYYY-MM-DD'), col('repPerDummyDate')).alias(
                'recordActivityDate'),
        ...
        course_type_counts = (registration_course_campus
            ...
            when(col('newCourseSectionLevel').isin('Undergraduate', 'Continuing Education', 'Other'), lit('UG'))
                ...
                .when(col('newCourseSectionLevel').isin('Professional Practice Doctorate'), lit('DPP')).alias(
                'newCourseSectionLevelUGGRDPP'))
                        ...
                        when(col('instructionalActivityType') == 'CR', col('newEnrollmentHours')).otherwise(
                            when(col('crseSectIsClockHours') == False, col('newEnrollmentHours')).otherwise(
                                when((col('crseSectIsClockHours') == True) & (col('instructionalActivityType') == 'B'),
                                     (col('newEnrollmentHours') * col('repPerEquivCRHRFactor'))).otherwise(
                                    col('newEnrollmentHours')))))
            .distinct()
            .groupBy(
            'regPersonId',
        ...
        if reporting_periods_in.rdd.isEmpty() == False:
            ...
            return reporting_periods_in


def student_cohort(spark, survey_info_in, default_values_in, ipeds_client_config_in=None, academic_term_in=None,
                   reporting_periods_in=None, course_type_counts_in=None):
    ...
    if not course_type_counts_in:
        if not reporting_periods_in:
            ...
            reporting_periods_in = reporting_periods(spark=spark, survey_info_in=survey_info_in,
                                                     ...
                                                     ipeds_reporting_period_in=ipeds_reporting_period_in,
                                                     academic_term_in=academic_term_in)
        course_type_counts_in = course_type_counts(spark=spark, survey_info_in=survey_info_in,
                                                   default_values_in=default_values_in,
                                                   ipeds_client_config_in=ipeds_client_config_in,
                                                   academic_term_in=academic_term_in,
                                                   reporting_periods_in=reporting_period_in)
    ...
    if (course_type_counts_in.rdd.isEmpty() == False) and (student_in.rdd.isEmpty() == False):
        ...
        student = (student_in
            ...
            to_timestamp(coalesce(student_in.recordActivityDate, col('repPerDummyDate'))).alias(
                'recordActivityDateTimestamp'),
            coalesce(to_date(student_in.recordActivityDate, 'YYYY-MM-DD'), col('repPerDummyDate')).alias(
                'recordActivityDate'),
        ...
        student_out = (student_reg
            ...
            when(col('survey_type') == '12ME', col('tmAnnualDPPCreditHoursFTE')).alias(
                'configTmAnnualDPPCreditHoursFTE'))
        ...
        academic_track = academic_track_cohort_mcr(spark, cohort_df_in=cohort_person, academic_term_in=academic_term_in,
                                                   priority_type_in='first')

        degree_program = degree_program_cohort_mcr(spark, cohort_df_in=academic_track,
                                                   academic_term_in=academic_term_in, data_type_in='all')
        ...
        cohort_priority = (degree_program
                           .withColumn('ipedsInclude',
                                       ...
                                       .when(((col('totalRemCourses') == col('totalCourses')) & (
                                                   col('isNonDegreeSeeking_final') == True)), lit(0))
                                       .when((col('degProgIsESL') == True) & (
                                           col('survey_type').isin(esl_enroll_surveys)), lit(0))
                                       .when((col('totalThesisCourses') > 0) & (
                                           col('survey_type').isin(graduate_enroll_surveys)), lit(1))
                                       .when((col('totalProfResidencyCourses') > 0) & (
                                           col('survey_type').isin(graduate_enroll_surveys)), lit(0))
                                       .when(((col('totalESLCourses') == col('totalCourses')) & (
                                                   col('isNonDegreeSeeking_final') == True)), lit(0))
            ...
            when(col('degreeLevel').isin('Masters', 'Doctorate'), lit('GR'))
                ...
                .otherwise(lit('UG')).alias('degreeLevelUGGRDPP'),
            # currently using studentLevelUGGRDPP for level filtering - consider using this field
            col('UGCreditHours'),
            col('UGClockHours'),
            col('GRCreditHours'),
            col('DPPCreditHours'),
        ...
        cohort_firstDegreeSeeking = (cohort_priority
            ...
            when((col('annualSurvey') == True) & (col('isNonDegreeSeeking') == False) & (col('NDSRn') == 1) & (
                        col('FFTRn') != 1)
        ...
        cohort_preFallSummer = (cohort_priority
            ...
            when((col('termType') == 'Pre-Fall Summer') & (col('studentLevelUGGRDPP') == 'UG') & (
                        col('isNonDegreeSeeking') == False), col('studentType')).alias('studentTypePreFallSummer'))
            .groupBy(
        ...
        cohort = (cohort_priority
            ...
            (col('personId') == col('personIdFirstDegreeSeeking')) & (
                        col('yearType') == col('yearTypeFirstDegreeSeeking')), 'left')
            .join(
            ...
            (col('personId') == col('personIdMaxStudentLevel')) & (col('yearType') == col('yearTypeMaxStudentLevel')),
            'left')
            .select(
            cohort_priority['*'],
