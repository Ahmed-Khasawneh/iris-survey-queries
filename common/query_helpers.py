...
def ipeds_course_type_counts():
    ...
    registration_course_section_schedule = registration_course_section.join(
        ...
        upper(
            coalesce(registration_course_section.regCourseSectionCampusOverride, course_section_schedule_in.campus)).alias(
...
def ipeds_cohort():
    ...
    student = student_in.join(
        broadcast(academic_term_reporting_refactor),
        ((upper(student_in.termCode) == academic_term_reporting_refactor.termCode)
        ...
        | (col('stuRecordActivityDate') == to_timestamp(lit('9999-09-09')))).withColumn(
        'studentRowNum',
        row_number().over(
            Window.partitionBy(
                col('repRefYearType'),
    ...
    cohort_person = student_fft_out.join(
        ...
        #     .when(((person_in.ethnicity == 'Hispanic or Latino') | (coalesce(person.isHispanic, lit(False))) == True), '2')
        ...
        to_timestamp(person_in.snapshotDate).alias('persSnapshotDate')).withColumn(
        'persRowNum',
        row_number().over(
            Window.partitionBy(
                col('repRefYearType'),
    ...
    academic_track = cohort_person.join(
        ...
        academic_term,
        (academic_term.termCode == academic_track_in.termCodeEffective) &
        ...
        to_timestamp(academic_track_in.snapshotDate).alias('acadTrkSnapshotDate')).withColumn(
        'acadTrkTrackRowNum',
        row_number().over(
            Window.partitionBy(
    ...
    degree_program = academic_track.join(
        ...
        academic_term,
        (academic_term.termCode == degree_program_in.termCodeEffective) &
        ...
        to_timestamp(degree_program_in.snapshotDate).alias('degProgSnapshotDate')).withColumn(
        'degProgRowNum',
        row_number().over(
            Window.partitionBy(
                col('repRefYearType'),
    ...
    degree = degree_program.join(
        ...
        to_timestamp(degree_in.snapshotDate).alias('degSnapshotDate')).withColumn(
        'degRowNum',
        row_number().over(
            Window.partitionBy(
                col('repRefYearType'),
    ...
    field_of_study = degree.join(
        ...
        ((coalesce(to_date(field_of_study_in.recordActivityDate, 'YYYY-MM-DD'), to_date(lit('9999-09-09'), 'YYYY-MM-DD')) == to_date(lit('9999-09-09'), 'YYYY-MM-DD'))
         | ((coalesce(to_date(field_of_study_in.recordActivityDate, 'YYYY-MM-DD'), to_date(lit('9999-09-09'), 'YYYY-MM-DD')) != to_date(lit('9999-09-09'), 'YYYY-MM-DD'))
            & (to_date(field_of_study_in.recordActivityDate, 'YYYY-MM-DD') <= to_date(degree.stuRefCensusDate, 'YYYY-MM-DD'))))
        ...
        to_timestamp(field_of_study_in.snapshotDate).alias('fldOfStdySnapshotDate')).withColumn(
        'fldOfStdyRowNum',
        row_number().over(
            Window.partitionBy(
                col('repRefYearType'),
