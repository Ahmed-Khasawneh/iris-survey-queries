...
FILE NAME:      Fall Enrollment v4 (EF3)
FILE DESC:      Fall Enrollment for private, 2-year and less than 2-year, non-degree-granting institutions
...
20200707		akhasawneh				ak 20200707			Modification to course/hour counts (PF-1552) -Run time 12m 45s
															Adding position status filter
															Adding employee assignment status filter
															Adding employee status filter
															Adding additional client config values for level offereings and instructor activity type
															Bug fixes in CourseMCR, CourseTypeCountsSTU, PersonMCR and mods to support changes to these views
...
20200429		akhasawneh				ak 20200419			Adding study abroad considerations to cohort and retention inclusions (PF-1435) - Run time 10m 56s
...
--EF3 mod - remove value 2
		else 1 --all undergraduate students
...
--undergraduate
	...
	select 'A' part,
		...
		ipedsLevel field2, -- Student level, 1,3,11,15,17, and 25 (refer to student level table, Part A) (8,14,22, 28, and 29 are for export only.)
	...
	from (
		...
		union
			...
			--EF3 mod - no Graduate level values 2, 7, 16, 21
			where StudentLevel.ipedsLevel != 2
				and StudentLevel.ipedsLevel != 7
				and StudentLevel.ipedsLevel != 16
				and StudentLevel.ipedsLevel != 21
	)
	group by ipedsLevel
...
union
	...
	select 'G', -- part,
		'2', -- sortorder,
		NULL, -- field1,
		ipedsLevel, -- field2, --Student level, 1 (refer to student level table (Part G) in appendix)
...
-- ak 20200413 (PF-1371) Mod to move all formatting to the 'Formatting Views'	
		...
		union
			...
			--EF3 mod - no Graduate level value 3 and no non-degree seeking undergrad value 2
			where StudentLevel.ipedsLevel = 1 
	)
	group by ipedsLevel
...
-- ak 20200413 (PF-1371) Mod to move all formatting to the 'Formatting Views'
		...
		union
		 ...
		 cross join FormatPartBAgeGroup AgeGroup
			--EF3 mod - no Graduate level value 3
...
union
	...
	select 'F', -- part,
		7, -- sortorder,
		NULL, -- field1,
		CAST(ROUND(NVL(fteemp.FTE, 0)/NVL(fteemp.FTE, 1)) as int), -- field2, --Student-to-faculty ratio, 0 - 100
		NULL, -- field3,
		NULL, -- field4,
		NULL, -- field5,
		NULL, -- field6,
