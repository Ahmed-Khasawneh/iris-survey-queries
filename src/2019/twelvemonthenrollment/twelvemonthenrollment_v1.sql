...
from ( 
	...
	from RegistrationMCR reg
		inner join Student studentENT on reg.personId = studentENT.personId 
			...
			and studentENT.studentStatus = 'Active' --do not report Study Abroad students	    		and studentENT.isIpedsReportable = 1
	)
where studRn = 1
),
...
select 'B'                                                                                                       part,
       ...
       coalesce(sum(cohortstu.totalCreditUGHrs), 0)                                                             field2, -- CREDHRSU - credit hour instructional activity at the undergraduate level, 0 to 99999999, blank = not applicable, if no undergraduate level programs are measured in credit hours.
       --case when useClockHours = 'None' then null else ...
       sum(cohortstu.totalClockHrs)                                                                              field3, -- CONTHRS  - clock hour instructional activity at the undergraduate level, 0 to 9999999, blank = not applicable, if no undergraduate programs are measured in clock hours.
       sum(cohortstu.totalCreditGRHrs)                                                                           field4, -- CREDHRSG - credit hour instructional activity at the graduate level, 0 to 99999999, blank = not applicable
       case
           when sum(cohortstu.totalCreditPostGRHrs) > 0
               then round((sum(cohortstu.totalCreditPostGRHrs)) / 18, 0) end                                    field5, -- RDOCFTE  - reported Doctor'92s degree-professional practice student FTE, 0 to 99999999, blank = not applicable
       null                                                                                                      field6,
       null                                                                                                      field7,
       null                                                                                                      field8,
       null                                                                                                      field9,
