...
select '2022' surveyYear, 
	...
	CAST('2021-08-01' AS DATE) reportingDateStart, --term start date
	CAST('2021-10-31' AS DATE) reportingDateEnd, --term end date
	'202210' termCode, --Fall 2020
	'1' partOfTermCode, 
	CAST('2021-10-15' AS DATE) censusDate,
...
from (
    ...
    from (
        select course1.personId,
                course1.yearType,
                (case when course1.studyAbroadStatus != 'Study Abroad - Home Institution' then course1.isNonDegreeSeeking
                      when course1.totalSAHomeCourses > 0 or course1.totalCreditHrs > 0 or course1.totalClockHrs > 0 then false 
                      else course1.isNonDegreeSeeking 
                  end) isNonDegreeSeeking,
                (case when course1.totalCECourses = course1.totalCourses then 0 --exclude students enrolled only in continuing ed courses
                    when course1.totalIntlCourses = course1.totalCourses then 0 --exclude students exclusively enrolled in any foreign branch campuses
                    when course1.totalAuditCourses = course1.totalCourses then 0 --exclude students exclusively auditing classes
                    when course1.totalProfResidencyCourses > 0 then 0 --exclude PHD residents or interns
                    when course1.totalThesisCourses > 0 then 0 --exclude PHD residents or interns
                    when course1.totalRemCourses = course1.totalCourses and course1.isNonDegreeSeeking = false then 1 --include students taking remedial courses if degree-seeking
                    when course1.totalESLCourses = course1.totalCourses and course1.isNonDegreeSeeking = false then 1 --exclude students enrolled only in ESL courses/programs
                    when course1.totalSAHomeCourses > 0 then 1 --include study abroad student where home institution provides resources, even if credit hours = 0
                    when course1.totalCreditHrs > 0 then 1
                    when course1.totalClockHrs > 0 then 1
                    else 0
                 end) ipedsInclude,
                course1.termCode,
                acadRep.maxCensus maxCensus,
                course1.termOrder,
                course1.snapshotDate,
                course1.instructionalActivityType,
                course1.requiredFTCreditHoursUG,
                course1.requiredFTClockHoursUG,
                course1.genderForUnknown,
                course1.genderForNonBinary,
                course1.studentLevel,
                course1.studentType,
                course1.isAdmitted,
                course1.isEnrolled,
                course1.totalClockHrs,
                course1.totalCreditHrs,
                course1.fullTimePartTimeStatus
        from (
        ...
        left join 
--Modification to address the error...    
--org.apache.spark.sql.catalyst.analysis.UnresolvedException: Invalid call to qualifier on unresolved object, tree: '    
            (select acadRep1.termCode, max(acadRep1.maxCensus) maxCensus
            from AcademicTermReportingRefactor acadRep1
            where acadRep1.partOfTermCode = acadRep1.maxPOT
            group by acadRep1.termCode) acadRep on acadRep.termcode = course1.termcode) 
...
from (
    select distinct 
			...
			crsecnt.instructionalActivityType instructionalActivityType,
			crsecnt.requiredFTCreditHoursUG requiredFTCreditHoursUG,
			crsecnt.requiredFTClockHoursUG requiredFTClockHoursUG,
			crsecnt.genderForUnknown genderForUnknown,
			crsecnt.genderForNonBinary genderForNonBinary,
			...
			crsecnt.totalClockHrs totalClockHrs,
			crsecnt.totalCreditHrs totalCreditHrs,
			crsecnt.fullTimePartTimeStatus fullTimePartTimeStatus,        
            ...
            coalesce(row_number() over (
                partition by
                    ...
                    crsecnt.yearType
                order by
                   (case when to_date(acadtrackENT.snapshotDate, 'YYYY-MM-DD') = crsecnt.snapshotDate then 1 else 2 end) asc,
                    (case when to_date(acadtrackENT.snapshotDate, 'YYYY-MM-DD') > crsecnt.snapshotDate then to_date(acadtrackENT.snapshotDate, 'YYYY-MM-DD') else CAST('9999-09-09' as DATE) end) asc,
                    (case when to_date(acadtrackENT.snapshotDate, 'YYYY-MM-DD') < crsecnt.snapshotDate then to_date(acadtrackENT.snapshotDate, 'YYYY-MM-DD') else CAST('1900-09-09' as DATE) end) desc,
    ...
    from CourseTypeCountsSTU crsecnt
        left join AcademicTrack acadtrackENT on crsecnt.personId = acadTrackENT.personId
            and ((coalesce(to_date(acadtrackENT.fieldOfStudyActionDate, 'YYYY-MM-DD'), CAST('9999-09-09' AS DATE)) != CAST('9999-09-09' AS DATE)
                        and to_date(acadtrackENT.fieldOfStudyActionDate,'YYYY-MM-DD') <= crsecnt.censusDate)
                    or (coalesce(to_date(acadtrackENT.fieldOfStudyActionDate, 'YYYY-MM-DD'), CAST('9999-09-09' AS DATE)) = CAST('9999-09-09' AS DATE)
                        and ((coalesce(to_date(acadtrackENT.recordActivityDate, 'YYYY-MM-DD'), CAST('9999-09-09' AS DATE)) != CAST('9999-09-09' as DATE)
                                and to_date(acadtrackENT.recordActivityDate,'YYYY-MM-DD') <= crsecnt.censusDate)
               ...
               left join AcademicTermOrder termorder on termorder.termCode = upper(acadtrackENT.termCodeEffective)
                                    and termorder.termOrder <= crsecnt.termOrder
        )
where acadTrackRn = 1
),
...
from (
    select distinct 
			acdtrk.personId personId,
			acdtrk.yearType yearType,
			acdtrk.censusDate censusDate,
			acdtrk.timeStatus timeStatus,
			acdtrk.isNonDegreeSeeking isNonDegreeSeeking,
			acdtrk.ipedsInclude ipedsInclude,
			acdtrk.termCode termCode,
			acdtrk.termOrder termOrder,
			acdtrk.snapshotDate snapshotDate,
			acdtrk.instructionalActivityType instructionalActivityType,
			acdtrk.requiredFTCreditHoursUG requiredFTCreditHoursUG,
			acdtrk.requiredFTClockHoursUG requiredFTClockHoursUG,
			acdtrk.genderForUnknown genderForUnknown,
			acdtrk.genderForNonBinary genderForNonBinary,
			acdtrk.studentLevel studentLevel,
			acdtrk.studentType studentType,
			acdtrk.isAdmitted isAdmitted,
			acdtrk.isEnrolled isEnrolled,
			acdtrk.totalClockHrs totalClockHrs,
			acdtrk.totalCreditHrs totalCreditHrs,
			acdtrk.fullTimePartTimeStatus fullTimePartTimeStatus,      
			acdtrk.personId atPersonId,
			acdtrk.degreeProgram degreeProgram,
			acdtrk.academicTrackStatus academicTrackStatus,
			acdtrk.fieldOfStudyActionDate fieldOfStudyActionDate,  
			acdtrk.recordActivityDate recordActivityDate,
			acdtrk.fieldOfStudyPriority fieldOfStudyPriority,
			acdtrk.isCurrentFieldOfStudy isCurrentFieldOfStudy,    
            ...
            coalesce(row_number() over (
                partition by
                    acdtrk.personId,
                    acdtrk.yearType,
                    acdtrk.degreeProgram,
                    degprogENT.degreeProgram
                order by
                    (case when to_date(degprogENT.snapshotDate, 'YYYY-MM-DD') = acdtrk.snapshotDate then 1 else 2 end) asc,
                    (case when to_date(degprogENT.snapshotDate, 'YYYY-MM-DD') > acdtrk.snapshotDate then to_date(degprogENT.snapshotDate, 'YYYY-MM-DD') else CAST('9999-09-09' as DATE) end) asc,
                    (case when to_date(degprogENT.snapshotDate, 'YYYY-MM-DD') < acdtrk.snapshotDate then to_date(degprogENT.snapshotDate, 'YYYY-MM-DD') else CAST('1900-09-09' as DATE) end) desc,
                    termorder.termOrder desc,
                    degprogENT.startDate desc,
                    degprogENT.recordActivityDate desc
            ), 1) degProgRn
    from AcademicTrackMCR acdtrk
        left join DegreeProgram degprogENT on acdtrk.degreeProgram = upper(degprogENT.degreeProgram)
                   and ((coalesce(to_date(degprogENT.recordActivityDate,'YYYY-MM-DD'), CAST('9999-09-09' as DATE)) != CAST('9999-09-09' as DATE)
                               and to_date(degprogENT.recordActivityDate,'YYYY-MM-DD') <= acdtrk.censusDate)
                         or coalesce(to_date(degprogENT.recordActivityDate,'YYYY-MM-DD'), CAST('9999-09-09' as DATE)) = CAST('9999-09-09' as DATE))
                                and (degprogENT.startDate is null 
                                    or to_date(degprogENT.startDate, 'YYYY-MM-DD') <= acdtrk.censusDate) 
         ...
         left join AcademicTermOrder termorder on termorder.termCode = upper(degprogENT.termCodeEffective)
                                and termorder.termOrder <= acdtrk.termOrder
        )
where degProgRn = 1
    --and isESL = false
),
...
from (
    select distinct 
            degprog.yearType yearType,
            --degprog.surveySection surveySection,
            --degprog.surveyYear surveyYear,
            --to_date(degprog.snapshotDate,'YYYY-MM-DD') snapshotDate,
            degprog.censusDate censusDate,
            degprog.termOrder termOrder,
            degprog.personId personId,
            degprog.isAdmitted isAdmitted,
            degprog.isEnrolled isEnrolled,
            --degprog.maxPartOfTermCode maxPartOfTermCode,
            degprog.ipedsInclude ipedsInclude, 
            degprog.termCode termCode, 
            degprog.studentLevel studentLevel, 
            degprog.studentType studentType, 
            degprog.isNonDegreeSeeking isNonDegreeSeeking, 
            degprog.snapshotDate snapshotDate, 
            degprog.timeStatus timeStatus,
            degprog.genderForUnknown,
            degprog.genderForNonBinary,
            degprog.isESL,
            ...
            coalesce(row_number() over (
                partition by
                    degprog.yearType,
                    --degprog.surveySection,
                    degprog.personId,
                    personENT.personId
                order by
                    (case when to_date(personENT.snapshotDate,'YYYY-MM-DD') = degprog.snapshotDate then 1 else 2 end) asc,
			        (case when to_date(personENT.snapshotDate, 'YYYY-MM-DD') > degprog.snapshotDate then to_date(personENT.snapshotDate,'YYYY-MM-DD') else CAST('9999-09-09' as DATE) end) asc, 
                    (case when to_date(personENT.snapshotDate, 'YYYY-MM-DD') < degprog.snapshotDate then to_date(personENT.snapshotDate,'YYYY-MM-DD') else CAST('1900-09-09' as DATE) end) desc,
                    coalesce(personENT.recordActivityDate, CAST('9999-09-09' as DATE)) desc
            ), 1) personRn
    from DegreeProgramMCR degprog 
        left join Person personENT on degprog.personId = personENT.personId
			and ((coalesce(to_date(personENT.recordActivityDate,'YYYY-MM-DD'), CAST('9999-09-09' AS DATE)) != CAST('9999-09-09' AS DATE)  
				and to_date(personENT.recordActivityDate,'YYYY-MM-DD') <= degprog.censusDate)
...
-- Number (of admitted) that enrolled full-time (Men)
	sum(case when timeStatus = 'FT' and ipedsGender = 'M' and isEnrolled = true and ipedsInclude = 1 and isESL = false then 1 end), --field7 
-- Number (of admitted) that enrolled full-time (Women)
	sum(case when timeStatus = 'FT' and ipedsGender = 'F' and isEnrolled = true and ipedsInclude = 1 and isESL = false then 1 end), --field8 
-- Number (of admitted) that enrolled full-time (Total)
	sum(case when timeStatus = 'FT' and isEnrolled = true and ipedsInclude = 1 and isESL = false then 1 end), --field9 
--Number (of admitted) that enrolled part-time (Male)
	sum(case when timeStatus = 'PT' and ipedsGender = 'M' and isEnrolled = true and ipedsInclude = 1 and isESL = false then 1 end), --field10 
--Number (of admitted) that enrolled part-time (Female)
	sum(case when timeStatus = 'PT' and ipedsGender = 'F' and isEnrolled = true and ipedsInclude = 1 and isESL = false then 1 end), --field11 
--Number (of admitted) that enrolled part-time (Total)
	sum(case when timeStatus = 'PT' and isEnrolled = true and ipedsInclude = 1 and isESL = false then 1 end), --field12 
	null, --field13
	null --field14
from PersonMCR 
